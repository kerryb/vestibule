...
Other than that, sounds great!
        ...
        context 'when there are some suggestions already' do
          ...
          should 'see the existing suggestions in the feed' do
            visit proposal_path(@proposal, :format => :rss)
            assert page.has_xpath?('.//item/title', :text => "Suggestion from #{@other_suggestion.author.name} (@#{@other_suggestion.author.ein})")
          ...
          should 'be able to add a new suggestion and see it in the feed at the top' do
            ...
            assert page.has_xpath?('.//item[position() = 2]/title', :text => "Suggestion from #{@other_suggestion.author.name} (@#{@other_suggestion.author.ein})")
            assert page.has_xpath?('.//item[position() = 2]/description', :text => @other_suggestion.body)

            assert page.has_xpath?('.//item[position() = 1]/title', :text => "Suggestion from #{@me.name} (@#{@me.ein})")
